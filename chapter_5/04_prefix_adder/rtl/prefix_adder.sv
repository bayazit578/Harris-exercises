module prefix_adder(
  ...
  assign prg [TERM_WIDTH-1:1] = a [TERM_WIDTH-2:0] | b [TERM_WIDTH-2:0];
  assign gen [TERM_WIDTH-1:1] = a [TERM_WIDTH-2:0] & b [TERM_WIDTH-2:0];
  ...
  generate
    for (genvar i = 0; i < TERM_WIDTH; i += 2) begin: g_pg_layer_0
      pg_cell u_pg_cell (
        ...
        .i_prg_low  (        prg [  i]),
        .i_gen_low  (        gen [  i]),
        .o_prg      (prg_layer_0 [i+1]),
        .o_gen      (gen_layer_0 [i+1])
      );
